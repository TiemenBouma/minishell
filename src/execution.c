...
int	exec2(struct	s_main *main_struct, int i)
{
	...
	if (i < main_struct->cmd_count)
	{
		...
		err_chk(pipe(main_struct->p[CUR]), 1, "");
		...
		{
			//printf("DEBUG1\n");
			main_struct->build_return =  exec_builtin(curr_cmd, is_builtin(curr_cmd->exec.exec_line[0]));
		}
		...
		main_struct->old_signal[0] = signal(SIGINT, sigint_handler_in_process);
		main_struct->old_signal[1] = signal(SIGQUIT, sigquit_handler_in_process);
		// printf("ID= %d\n", id);
		err_chk(main_struct->id, 1, "");
		if (main_struct->id == 0 && check_buildin_fork(curr_cmd) == 1)//Why is is_builtin in this if statment
		{
			// printf("EXEC1\n");
			...
			else
			{
				...
				err_chk(dup2(main_struct->p[PREV][P_OUT], S_IN), 2, "");	
			}
			if (curr_cmd->exec.fd_out == 1 && main_struct->cmd_count != i)
			{
				// printf("EXEC5\n");
				
				err_chk(dup2(main_struct->p[CUR][P_IN], S_OUT), 2, "");
			}
			else
			{
				// printf("EXEC6\n");
		...
		if (i > 1)
		{
			...
			close (main_struct->p[PREV][P_OUT]);
		}
		//printf("close (p[CUR][P_IN]);%d\n", p[CUR][P_IN]);
		// printf("EXEC11\n");
		close (main_struct->p[CUR][P_IN]);
		// printf("EXEC12\n");
